...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <!-- Top header -->
      <div>
        <h2 class="text-2xl font-bold text-text-primary">{{ 'team.title' | translate }}</h2>
        <p class="text-text-secondary text-sm">{{ 'team.desc' | translate }}</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-surface border border-border p-5 rounded-2xl shadow-sm space-y-4">
          <h3 class="font-bold text-text-primary text-base pb-2 border-b border-border">{{ 'team.inviteTitle' | translate }}</h3>
          <p class="text-xs text-text-secondary">{{ 'team.inviteDesc' | translate }}</p>
          
          <form (submit)="onSendInvitations($event)" class="space-y-4">
            <div>
              <label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase">{{ 'team.emailsLabel' | translate }}</label>
              ...
              <p class="text-[10px] text-text-secondary mt-1.5">{{ 'team.pressEnter' | translate }}</p>
            </div>
            ...
            <button type="submit" 
              ...
              @if (isInviting()) {
                {{ 'team.sendingInvites' | translate }}
              } @else {
                {{ 'team.sendInvitations' | translate }}
              }
            </button>
          </form>
        </div>
        ...
        <div class="bg-surface border border-border p-5 rounded-2xl shadow-sm space-y-4 lg:col-span-2">
          <div class="flex items-center justify-between pb-2 border-b border-border flex-wrap gap-2">
            <h3 class="font-bold text-text-primary text-base">{{ 'team.activeAssignment' | translate }}</h3>
            @if (projectState.selectedProjectId()) {
              ...
              <span class="text-xs text-red-500 font-bold">{{ 'team.selectProjectFirst' | translate }}</span>
            }
          </div>
          ...
          @if (!projectState.selectedProjectId()) {
            <div class="flex flex-col items-center justify-center py-12 text-center text-text-secondary text-sm">
              ...
              {{ 'team.selectProjectPrompt' | translate }}
            </div>
          } @else {
            <!-- Error Banner -->
            @if (assignError()) {
              <div class="mb-4 p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-start gap-2.5 text-red-600 dark:text-red-400 text-xs animate-[fadeIn_0.2s_ease_both]">
                ...
                <div class="flex-1">
                  <p class="font-extrabold text-sm text-red-700 dark:text-red-300">{{ 'team.assignmentFailed' | translate }}</p>
            ...
            @if (projectState.selectedProject()?.status !== 'Completed' && projectState.selectedProject()?.status !== 'Archived') {
              ...
              <div class="relative">
                <label class="block text-xs font-bold text-text-secondary mb-1.5">{{ 'team.selectMember' | translate }}</label>
                ...
                <div (click)="isDropdownOpen.set(!isDropdownOpen())"
                  ...
                  @if (selectedEmployeeId) {
                    ...
                    <span class="text-text-secondary">{{ 'team.chooseMember' | translate }}</span>
                  }
                    ...
                    @for (emp of companyEmployees(); track emp.employeeId) {
                      <div (click)="selectEmployee(emp.employeeId)"
                        ...
                        @if (failedEmployeeIds().includes(emp.employeeId)) {
                          <span class="text-[10px] text-red-500 font-bold ms-auto px-1.5 py-0.5 bg-red-500/10 rounded border border-red-500/20 shrink-0">
                            {{ 'team.alreadyInProject' | translate }}
                          </span>
              ...
              <div>
                ...
                <select [(ngModel)]="assignedRole" name="assignRole" required
                  ...
                  <option value="Developer">{{ 'team.dev' | translate }}</option>
                  <option value="QA">{{ 'team.qa' | translate }}</option>
                  <option value="Scrum Master">{{ 'team.scrum' | translate }}</option>
                  <option value="Product Owner">{{ 'team.po' | translate }}</option>
                  <option value="Designer">{{ 'team.design' | translate }}</option>
                </select>
              </div>
              ...
              <div class="flex items-end">
                <button type="submit" 
                  ...
                  {{ 'team.assignToProject' | translate }}
                </button>
              </div>
              </form>
            }
            ...
            <div class="space-y-3 mt-4">
              <h4 class="text-xs font-bold text-text-secondary uppercase tracking-wider">{{ 'team.assignedTeam' | translate }} ({{ projectTeam().length }})</h4>
                  ...
                  @for (member of projectTeam(); track member.employeeId) {
                    <div class="bg-sidebar border border-border p-4 rounded-xl flex items-center justify-between gap-3 hover:border-primary/20 transition-all">
                      ...
                      <span class="px-2.5 py-1 text-[10px] font-bold bg-primary/10 text-primary border border-primary/20 rounded-full">
                        {{ member.role ? ('roles.' + member.role | translate) : member.role }}
                      </span>
                    </div>
                  }
                </div>
