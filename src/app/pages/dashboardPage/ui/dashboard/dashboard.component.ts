        ...
        <div class="border-t border-border pt-6 mt-6 space-y-4">
          <div routerLink="/dashboard/profile" class="cursor-pointer flex items-center bg-surface border border-border rounded-xl transition-all duration-250 hover:border-primary/40 hover:shadow-sm"
            ...
            <div class="min-w-0" [class.hidden]="isSidebarCollapsed()">
              ...
              <p class="text-[10px] text-text-secondary truncate">{{ userJobTitle() ? (userJobTitle() | translate) : '' }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="fixed bottom-2 left-2 right-2 z-40 bg-surface/75 backdrop-blur-xl border border-border flex items-center justify-start overflow-x-auto flex-nowrap gap-1 px-3 py-1.5 md:hidden rounded-2xl shadow-xl transition-all duration-300 [&>button]:shrink-0 [&>button]:min-w-[3.5rem]" style="scrollbar-width: none;">
        ...
        @if (projectState.isProjectManager()) {
          <button routerLink="/dashboard/projects" routerLinkActive="text-primary scale-105" #rlaProjM="routerLinkActive"
            ...
            <span class="text-[9px] font-bold">{{ 'SIDEBAR.ALL_PROJECTS' | translate }}</span>
          </button>
        }
        ...
        <button routerLink="/dashboard/backlog" routerLinkActive="text-primary scale-105" #rlaBacklogM="routerLinkActive"
          ...
          <span class="text-[9px] font-bold">{{ 'SIDEBAR.BACKLOG' | translate }}</span>
        </button>
        ...
        @if (projectState.isProjectManager()) {
          <button routerLink="/dashboard/sprint-planning" routerLinkActive="text-primary scale-105" #rlaSprintPlanM="routerLinkActive"
            ...
            <span class="text-[9px] font-bold">{{ 'SIDEBAR.SPRINT_PLANNING' | translate }}</span>
            <span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-primary animate-pulse"></span>
          </button>
        }

        <!-- Retrospective Tab (Mobile) -->
        <button routerLink="/dashboard/retrospective" routerLinkActive="text-primary scale-105" #rlaRetroM="routerLinkActive"
           [class.text-text-secondary]="!rlaRetroM.isActive"
           class="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-all duration-200 relative">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
          </svg>
          <span class="text-[9px] font-bold">{{ 'SIDEBAR.RETROSPECTIVE' | translate }}</span>
          <span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-primary animate-pulse"></span>
        </button>

        <!-- Project Team Tab (Mobile PM only) -->
        @if (projectState.isProjectManager()) {
          <button routerLink="/dashboard/team" routerLinkActive="text-primary scale-105" #rlaTeamM="routerLinkActive"
            ...
            <span class="text-[9px] font-bold">{{ 'SIDEBAR.PROJECT_TEAM' | translate }}</span>
          </button>

          <!-- Project Policies Tab (Mobile PM only) -->
          <button routerLink="/dashboard/project-policies" routerLinkActive="text-primary scale-105" #rlaProjPolM="routerLinkActive"
            ...
            <span class="text-[9px] font-bold">{{ 'SIDEBAR.PROJECT_POLICIES' | translate }}</span>
            <span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-primary animate-pulse"></span>
          </button>

          <!-- Employees Tab (Mobile PM only) -->
        ...
        <!-- Organization Hub Tab (Mobile) -->
        <button routerLink="/dashboard/organization" routerLinkActive="text-primary scale-105" #rlaOrgM="routerLinkActive"
           [class.text-text-secondary]="!rlaOrgM.isActive"
           class="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-all duration-200">
        ...
        <button routerLink="/dashboard/profile" routerLinkActive="text-primary scale-105" #rlaProfileM="routerLinkActive"
          ...
          <span class="text-[9px] font-bold">{{ 'SIDEBAR.MY_PROFILE' | translate }}</span>
        </button>
