...
@Component({
  ...
  template: `
    <div
      ...
      [class.z-50]="!embedded()"
      [class.p-4]="!embedded()"
      [class.bg-black/60]="!embedded()"
      [class.backdrop-blur-sm]="!embedded()"
      [class.w-full]="embedded()"
    >
      <div
        class="bg-surface border border-border rounded-3xl w-full flex flex-col shadow-sm overflow-hidden animate-[scaleUp_0.25s_ease_both]"
        [class.max-w-3xl]="!embedded()"
        [class.h-[85vh]]="!embedded()"
        [class.max-w-none]="embedded()"
        [class.min-h-[520px]]="embedded()"
      >
        <!-- Header -->
        <div
          class="p-5 border-b border-border bg-sidebar flex items-center justify-between shrink-0"
        >
          <div class="flex items-center gap-3">
            ...
            >
              <svg
                class="w-5.5 h-5.5 animate-pulse"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <button
            (click)="close.emit()"
            class="p-2 hover:bg-border rounded-full transition-colors text-text-secondary"
          >
        ...
        @if (projectState.isProjectManager()) {
          <div
            class="px-6 py-4 bg-primary/5 border-b border-primary/10 shrink-0 flex items-center justify-between gap-4"
          >
            <div class="flex-1">
        ...
        >
          ...
          @if (clarifyingQuestions().length > 0 && projectState.isProjectManager()) {
            <div
              class="p-5 bg-warning/5 border border-warning/20 rounded-2xl space-y-2.5 animate-[fadeIn_0.3s_ease_both]"
            >
              <h4
                class="text-xs font-bold text-warning uppercase tracking-wider flex items-center gap-1.5"
              >
          ...
          @if (isGeneratingDraft()) {
            <div
              class="flex items-center gap-2 text-primary font-semibold text-sm animate-pulse p-4"
            >
              <div
        ...
        <div class="p-4 border-t border-border bg-sidebar shrink-0 space-y-3">
          ...
          <div class="flex items-center gap-2">
            ...
            @if (selectedFileName()) {
              ...
              <button (click)="clearFile()" class="text-xs text-red-500 font-bold hover:underline">
                Remove
              </button>
            }
          </div>
          <form (submit)="onSendMessage($event)" class="flex gap-2 items-end">
            <textarea
              ...
              class="flex-1 bg-background border border-border rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20 transition-all resize-none"
            ></textarea>
            <button
              type="submit"
        ...
        @if (showNamePrompt()) {
          <div
            class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-[fadeIn_0.2s_ease_both]"
          >
            <div
              class="bg-surface border border-border rounded-3xl w-full max-w-2xl p-6 shadow-2xl flex flex-col gap-5 animate-[scaleUp_0.25s_ease_both] overflow-hidden"
            >
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 bg-primary/10 text-primary rounded-xl flex items-center justify-center text-lg font-bold"
                >
                  📂
                </div>
                <div>
                  <h4 class="text-base font-bold text-text-primary">
                    Configure & Initialize Project Workspace
                  </h4>
                  <p class="text-xs text-text-secondary">
                    Provide initial metadata and sprint settings to create the workspace.
                  </p>
                </div>
              </div>
              ...
              <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label
                      class="block text-[11px] font-extrabold text-text-secondary mb-1 uppercase tracking-wider"
                      >Project Name (English)</label
                    >
                    <input
                      type="text"
                      [value]="projectNameInput()"
                      (input)="projectNameInput.set(nameEnField.value)"
                      #nameEnField
                  ...
                  <div>
                    <label
                      class="block text-[11px] font-extrabold text-text-secondary mb-1 uppercase tracking-wider"
                      >اسم المشروع (عربي)</label
                    >
                    <input
                      type="text"
                      [value]="projectNameArInput()"
                      (input)="projectNameArInput.set(nameArField.value)"
                      #nameArField
                      dir="rtl"
                      [disabled]="isGeneratingDraft()"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                    <textarea
                      [value]="projectDescriptionEnInput()"
                      (input)="projectDescriptionEnInput.set(descEnField.value)"
                      #descEnField
                      rows="3"
                      [disabled]="isGeneratingDraft()"
                      ...
                      placeholder="Describe the project goal, core features, and target audience..."
                    ></textarea>
                  </div>
                  <div>
                    <label
                    ...
                    >
                    <textarea
                      [value]="projectDescriptionArInput()"
                      (input)="projectDescriptionArInput.set(descArField.value)"
                      #descArField
                      rows="3"
                      dir="rtl"
                      [disabled]="isGeneratingDraft()"
                      ...
                      placeholder="اكتب وصفاً مختصراً لأهداف ومميزات هذا المشروع..."
                    ></textarea>
                  </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-border/60 pt-4">
                  <div>
                    ...
                    <input
                      ...
                      (input)="sprintDurationInput.set(+durationField.value || null)"
                      #durationField
                      [disabled]="isGeneratingDraft()"
                      min="1"
                      max="90"
                  ...
                  <div>
                    <label
                      class="block text-[11px] font-extrabold text-text-secondary mb-1 uppercase tracking-wider"
                      >Target Sprint Hours</label
                    >
                    <input
                      type="number"
                      [value]="targetSprintHoursInput() ?? ''"
                      (input)="targetSprintHoursInput.set(+hoursField.value || null)"
                      #hoursField
                      [disabled]="isGeneratingDraft()"
                      min="1"
                      max="1000"
              ...
              <div
                class="flex items-center justify-end gap-2.5 mt-2 border-t border-border/60 pt-4 shrink-0"
              >
                <button
                  (click)="showNamePrompt.set(false)"
                  [disabled]="isGeneratingDraft()"
                ...
                >
                  @if (isGeneratingDraft()) {
                    <span
                      class="w-3.5 h-3.5 rounded-full border-2 border-white border-t-transparent animate-spin inline-block"
                    ></span>
...
export class AiChatModalComponent implements AfterViewChecked {
  ...
  async onSendMessage(event: Event) {
    ...
    try {
      ...
      const res = await this.aiRequirements.startOrContinueSession(
        userText,
        this.selectedFile(),
        this.chatId(),
      );
  ...
  async pollStatus(chatId: string) {
    try {
      ...
      if (data) {
        ...
        const sprintDays =
          data.suggestedSprintDurationInDays ||
          data.sprintDurationInDays ||
          data.SprintDurationInDays ||
          null;
        const sprintHours =
          data.suggestedTargetSprintHours ||
          data.targetSprintHours ||
          data.TargetSprintHours ||
          null;
