...
@Component({
  ...
  imports: [CommonModule, AiTelemetryComponent, TranslatePipe],
  template: `
    <div class="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8 animate-fade-in">
      ...
      <div class="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col md:flex-row md:items-center justify-between p-6 gap-6 transition-all duration-300 hover:shadow-md">
        <div>
          <h2 class="text-2xl font-extrabold text-text-primary">{{ 'SETTINGS.TITLE' | translate }}</h2>
          <p class="text-text-secondary text-sm font-medium">{{ 'SETTINGS.DESC' | translate }}</p>
        </div>
      </div>

      <!-- Integrations Section -->
      <div class="bg-surface border border-border rounded-2xl shadow-sm p-6 space-y-4 transition-colors duration-200">
        <h3 class="font-bold text-text-primary text-lg pb-2 border-b border-border">{{ 'SETTINGS.CONNECTED_ACCOUNTS' | translate }}</h3>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
          <div>
            ...
            <p class="text-sm text-text-secondary mt-1">
              {{ 'SETTINGS.GOOGLE_CALENDAR_DESC' | translate }}
            </p>
          </div>
          ...
          <button 
            ...
            @if (isConnectingCalendar()) {
              ...
              {{ 'SETTINGS.CONNECTING' | translate }}
            } @else {
              {{ 'SETTINGS.CONNECT_ACCOUNT' | translate }}
            }
          </button>
        </div>
      </div>
