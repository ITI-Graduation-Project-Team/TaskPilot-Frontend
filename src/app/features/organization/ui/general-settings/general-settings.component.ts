    ...
    <div class="bg-surface rounded-3xl border border-border p-6 md:p-8 shadow-sm">
      <div class="mb-6">
        ...
        <p class="text-sm text-text-secondary mt-1">
          {{ currentLang === 'ar' ? 'قم بإدارة الهوية الأساسية لشركتك وعلامتها التجارية.' : 'Manage your company\\'s core identity and branding.' }}
        </p>
      </div>
      ...
      <div class="grid gap-8 lg:grid-cols-[1fr_auto]">
        ...
        <form class="space-y-5" (submit)="saveSettings($event)">
          ...
          <div>
            <label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase tracking-wider">
              {{ currentLang === 'ar' ? 'اسم الشركة' : 'Company Name' }}
            </label>
            <input type="text" [(ngModel)]="companyName" name="companyName" required [placeholder]="currentLang === 'ar' ? 'مثال: شركة أكرو' : 'e.g. Acme Corp'" 
        ...
        <div class="lg:border-l lg:border-border lg:pl-8 flex flex-col items-center">
          <p class="text-xs font-bold text-text-secondary mb-4 uppercase tracking-wider text-center">
            {{ currentLang === 'ar' ? 'شعار الشركة' : 'Company Logo' }}
          </p>
          
          <div class="relative group cursor-pointer">
            <!-- Current Logo or Placeholder -->
            <div class="w-32 h-32 rounded-2xl bg-sidebar border-2 border-dashed border-border flex items-center justify-center overflow-hidden transition-all group-hover:border-primary/50 relative">
              @if (companyLogo()) {
            ...
            <div class="absolute inset-0 bg-black/40 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <span class="text-white text-xs font-bold">{{ currentLang === 'ar' ? 'تغيير الشعار' : 'Change Logo' }}</span>
            </div>
            
            <!-- Hidden File Input -->
            <input type="file" accept="image/*" (change)="onLogoUpload($event)" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10" title="Upload new logo">
          </div>
          <p class="text-[10px] text-text-secondary mt-3 text-center max-w-[140px]">
            {{ currentLang === 'ar' ? 'يوصى به: صورة مربعة، SVG أو PNG.' : 'Recommended: Square image, SVG or PNG.' }}
          </p>
        </div>
      </div>
    </div>
  ...
  async saveSettings(event: Event) {
    ...
    setTimeout(() => {
      this.toastService.show(
        this.currentLang === 'ar' ? 'تم حفظ الإعدادات بنجاح' : 'Settings saved successfully', 
        'success'
      );
